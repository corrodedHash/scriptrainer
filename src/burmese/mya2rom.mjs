...
import { vowels, consonants, vowels_in, specials } from './romanisations.mjs'

// eslint-disable-next-line no-unused-vars
const m2r_VERSION = Object.freeze('0.4.4')
 ...
 * */
export const SYSTEMS = Object.freeze(['ipa', 'mlcts', 'mlcts2', 'simple', 'simple2'])
 ...
 * @returns {string}
 */
export function mya2rom(word, system, show_nice_alts = false, is_manual = false) {
  ...
  for (var i = 0; i < syllables.length; i++) {
    ...
    // Check for MEDIALS
    if (medicoda.search('ှ') != -1) {
      // IPA uses under-ring, for devoice
      //syllable_ipa += "\u0325";
    ...
    for (const letter in vowels) {
      ...
      if (search_result != -1) {
        ...
        if (vowels_in[initial] == undefined && specials[initial] == undefined) {
          // We only add a vowel if there isn't an independent vowel or special letter
...
export function to_syllables(word) {
  var syllables = []
  ...
  for (const letter in vowels_in) {
    consonant_str += letter
  }
  for (const letter in specials) {
    consonant_str += letter
  }
...
function get_consonant(letter) {
  ...
  if (consonants[letter] != undefined) {
    // consonant?

    consonant_ipa += consonants[letter][ridx]
  } else if (vowels_in[letter] != undefined) {
    // or independent vowel?

    consonant_ipa += vowels_in[letter][ridx]
  } else if (specials[letter] != undefined) {
    // or special letter?
 ...
 *
 */
export function mya2rom_all(word, show_nice_alts = false, is_manual = false) {
  var converted_arr = []
